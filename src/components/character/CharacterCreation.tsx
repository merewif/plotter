...
const Charactercreation = () => {
  ...
  const [display, setDisplay] = useState<React.ReactElement | null>(null);
  ...
  function stepHandling(step: number) {
    // $(textareaClass).show();
    if (step > 4) {
      setText('');
  ...
  function saveCharacter() {
    ...
    const newCharMergedWithStoredChars = {
      ...storedCharObject,
      [name.replace(/\s+/g, '-').toLowerCase()]: newCharacter,
    };

    setTimeout(() => {
      localStorage.setItem('characters', JSON.stringify(newCharMergedWithStoredChars));
    }, 2000);
    setTimeout(() => {
      setDisplay(<CharacterDisplay currentCharacter={newCharacter} editable={false} />);
  ...
  function characterSaved() {
    setTimeout(() => {
      setText("All done! You'll be able to access and edit your character information later.");
      setSaveButtonText('Save character');
    }, 2000);
  ...
  function textInput(event: React.ChangeEvent<HTMLTextAreaElement>) {
